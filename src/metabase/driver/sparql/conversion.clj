...
(def ^:private mixed-type-resolution
  "When a column's observed base types differ, the most specific type that can
   ...
   3+-type mix, e.g. #{Integer Float Text} — deliberately degrades to Text,
...
(defn determine-column-types
  "Determines column types from the result rows.
   ...
   are already fully materialized in memory, and per column only the distinct
   (type, datatype) pairs (typically one or two) are classified, so the scan
   stays cheap even on large results.
  ...
  (reduce (fn [types var-name]
            (let [var-key (keyword var-name)
                  ;; Distinct raw (type, datatype) pairs across all rows;
                  ;; classification runs on this small set, not per cell.
                  pairs        (into #{}
                                     (keep (fn [row]
